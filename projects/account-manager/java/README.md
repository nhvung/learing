# Account Manager — Java Spring Boot

A fullstack account management app built with Spring Boot 3.2, JDBC, and PostgreSQL. The frontend is a single-page app served as static files by Spring Boot.

## Prerequisites

- Java 21 + Maven (for local run)
...
```bash
DB_HOST=localhost DB_PORT=5432 DB_NAME=account_manager DB_USER=postgres DB_PASSWORD=postgres mvn spring-boot:run
```
